...
import { User, Calendar, MessageCircle, X, Search, Users, Brain, Clock, Tag } from 'lucide-react';

const ContextualApp = () => {
  const [contacts, setContacts] = useState<any[]>([]);
  const [interactions, setInteractions] = useState<any[]>([]);
  const [unassignedInsights, setUnassignedInsights] = useState<any[]>([]);
...
Yeah, been learning for a few months now. I just wanted to try something new after school, you know?`,
  ...
  const confirmInsight = (insight: any, contactId: number | null = null) => {
    if (contactId) {
  ...
  const assignInsightToContact = (insight: any, contactId: number) => {
    setContacts(prev => prev.map(contact => 
      contact.id === contactId 
                      ...
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          <div className="flex space-x-2 mt-2">
                            {interaction.extractedInsights.map((insight: any, idx: number) => (
                ...
                <div key={contact.id} className="bg-white rounded-lg shadow hover:shadow-md transition-shadow">
                  <div className="p-6">
                    ...
                    <div className="space-y-3">
                      ...
                      {contact.facts.slice(0, 3).map((fact: any) => (
